# Prototype

## Introdução

O Prototype é a 4° etapa de Design Sprint, que consiste em criar um protótipo navegável baseado nos resultados da etapa de [Decide](https://unbarqdsw2026-1-turma01.github.io/Grupo02_ConhecendoRequisitos_Entrega01/#/Base/1.1.3.Decide). O protótipo é uma representação inicial de um design, criada para testar e validar ideias antes do desenvolvimento final. Ele permite que os usuários interajam com o sistema e avaliem sua usabilidade e aparência, identificando possíveis falhas e melhorias. O protótipo pode variar de esboços simples a versões digitais de alta fidelidade, permitindo simular soluções antes da implementação. Essa prática reduz custos e retrabalho, além de garantir que o design e as funcionalidades atendam às necessidades reais dos usuários[[1]](#bibliografia).

## Objetivos

Verificar a usabilidade, a compreensibilidade e a adequação das telas desenvolvidas para o site ConhecendoRequisitos, identificando problemas de interação e oportunidades de melhoria antes de avançar para a implementação final.

## Participantes

Tabela 1: Participantes da elaboração do protótipo

| Matrícula | Aluno              |
| --------- | ------------------ |
| 231027032 | Arthur Oliveira    |
| 231037692 | Isabella Choukaira |
| 231035455 | Leticia Jesus      |
| 231038303 | Yan Aguiar         |
| 231012316 | Yasmin Nascimento  |

## Protótipo de alta fidelidade

<iframe style="border: 1px solid rgba(0, 0, 0, 0.1);" width="800" height="450" src="https://embed.figma.com/design/2fL2A75XSM7oaHx2yJXJ8K/ConhecendoRequisitos?node-id=0-1&embed-host=share" allowfullscreen></iframe>

Acompanhe com mais detalhes cada tela produzida abaixo:

### Login / Cadastro

O usuário deve criar uma conta para acessar o site, para armazenar seu progresso de aprendizado.

<div align="center">

<b> Figura 1:</b> Login

</div>

![prototype-login](../images/Login.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 2:</b> Cadastro

</div>

![prototype-cadastro](../images/cadastro.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 3:</b> Esqueceu a senha

</div>

![prototype-esqueceu-senha](../images/Recuperar-senha.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

### Página inicial / Trilhas

O usuário poderá acessar suas trilhas de conteúdos, sabendo quais finalizou, quais estão em andamento ou finalizadas. As trilhas formam um caminho lógico no aprendizado de requisitos de software.

<div align="center">

<b> Figura 4:</b> Trilhas disponíveis

</div>

![prototype-trilhas](../images/trilhas.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

### Explorar a trilha

O usuário poderá entrar em cada trilha e explorar seus módulos, sendo sub-etapas obrigatórias (módulos) para o aprendizado de algo mais amplo (trilhas)

<div align="center">

<b> Figura 5:</b> Módulo da trilha

</div>

![prototype-modulos](../images/modulo.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

### Conteúdo

O usuário poderá acessar o conteúdo do módulo, onde terá acesso à video-aulas ou explicações em texto sobre algum conteúdo expecífico.

<div align="center">

<b> Figura 6:</b> Conteúdo

</div>

![prototype-conteudos](../images/conteudo.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

### Quiz

Ao final do conteúdo, o usuário deve responder ao um quiz para comprovar seu avanço de aprendizado no módulo, tendo feedbacks constantes de respostas certas e erradas, caso consiga mais de 70% de acertos, o usuário desbloqueia o acesso ao próximo módulo da trilha.

<div align="center">

<b> Figura 7:</b> Quiz

</div>

![prototype-quiz-intro](../images/Quiz-intro.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 8:</b> Quiz questao 1

</div>

![prototype-quiz-questao1](../images/Quiz-q1.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 9:</b> Quiz acertou

</div>

![prototype-quiz-acertou](../images/Quiz-acertou.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 10:</b> Quiz questão 5

</div>

![prototype-quizq5](../images/quiz-q5.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 11:</b> Quiz errou

</div>

![prototype-quiz-errou](../images/quiz-errou.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

<div align="center">

<b> Figura 12:</b> Quiz continuar

</div>

![prototype-quiz-continuar](../images/Quiz-continuar.png)

<div align="center">

_Fonte: elaborado pelos autores (2026)._

</div>

## Conclusão

Esse protótipo consolida, em uma versão navegável, as principais decisões tomadas na etapa de [Decide](https://unbarqdsw2026-1-turma01.github.io/Grupo02_ConhecendoRequisitos_Entrega01/#/Base/1.1.3.Decide) e permite avaliar a experiência proposta antes da implementação final. A partir dele, a equipe tem uma base concreta para observar a interação do usuário, identificar ajustes necessários e validar se as escolhas de interface atendem aos objetivos definidos.

Dessa forma, o Prototype se torna o ponto de partida para a etapa de [Test](https://unbarqdsw2026-1-turma01.github.io/Grupo02_ConhecendoRequisitos_Entrega01/#/Base/1.1.5.Test), na qual o protótipo será submetido à avaliação para verificar sua usabilidade e aderência às necessidades dos usuários.

## Bibliografia

1. ROGERS, Yvonne; SHARP, Helen; PREECE, Jennifer. Design de Interação: Além da Interação Humano-Computador. 3. ed. Porto Alegre: Bookman, 2013. xiv, 585 p.Cap 8 Acesso em: 5 mar. 2025

## Histórico de versões

| Versão | Data       | Descrição            | Autores                                          | Revisor |
| ------ | ---------- | -------------------- | ------------------------------------------------ | ------- |
| 1.0    | 04/04/2026 | Criação do documento | [Yan Matheus](https://github.com/Yanmatheus0812) |    [Arthur Evangelista](https://github.com/arthurevg)     |
| 1.1    | 05/04/2026 | Adição de Hyperlinks ligando este artefato ao Decide e ao Test | [Arthur Evangelista](https://github.com/arthurevg) |    [Yan Matheus](https://github.com/Yanmatheus0812)     |